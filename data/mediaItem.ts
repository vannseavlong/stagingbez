export const mediaItems = [
  // {
  //   id: 1,
  //   image: "/images/media/makro/m1.webp",

  //   images: [
  //         "/images/media/makro/m1.webp",
  //        "/images/media/makro/m2.webp",
  //        "/images/media/makro/m3.webp",
  //        "/images/media/makro/m4.webp",
  //        "/images/media/makro/m5.webp",
  //        "/images/media/makro/m6.webp",
  //        "/images/media/makro/m7.webp",
  //        "/images/media/makro/m8.webp"
  //      ],
  //   date: "May 24-25 2025",
  //   title: "bEasy Event at Makro",
  //   description:
  //     "Thank You for Visiting the bEasy Booth at Makro Sen Sok. On May 24–25, 2025, our Sales and Marketing Team set up a booth in front of Makro Sen Sok from 3 PM to 9 PM to actively introduce and demonstrate the bEasy App to the community. The event provided the perfect opportunity to test the app in a real-world setting, gather valuable feedback from users, and observe how people interact with its features. Even with the heavy rain, many attendees tried the app, explored its services, and shared their impressions—helping us identify ways to make the experience even more seamless and user-friendly. These hands-on interactions are essential to refining bEasy, ensuring it meets the needs of households and businesses across Cambodia. As we continue to host similar events, every test and conversation brings us closer to delivering the most convenient and reliable service platform possible.",
  // },

//   {
//     id: 1,
//     title: "What is bEasy?",
//     date: "11 Nov 2025",
//     image: "/images/beasy-cover.png",
//     highlight: "In today’s fast-paced life, keeping your home or office clean can feel overwhelming.",
//     body: `In today’s fast-paced life, keeping your home or office clean can feel overwhelming. 
// That’s where the bEasy App comes in – Cambodia’s all-in-one platform for professional cleaning and maintenance services.`,
//     sections: [
//       {
//         heading: "1. Cleaning",
//         content: "With the help of our general cleaning service..."
//       },
//       {
//         heading: "2. Deep Cleaning",
//         content: "An in-depth cleaning service for anyone..."
//       },
//       // etc.
//     ],
//   },
  // {
  //   id: 2,
  //   image: "/images/media/axis/a1.webp",
  //   images: [
  //         "/images/media/axis/a1.webp",
  //        "/images/media/axis/a2.webp",
  //        "/images/media/axis/a3.webp",
  //        "/images/media/axis/a4.webp",
  //        "/images/media/axis/a5.webp",
  //        "/images/media/axis/a6.webp",
  //        "/images/media/axis/a7.webp",
  //        "/images/media/axis/a8.webp"
  //      ],
  //   date: "May 17 2025",
  //   title: "bEasy app promotion at AXIS Residence",
  //   description:
  //     "Our bEasy App brings multiple essential services together on one platform—whether you need cleaning, moving assistance, or beauty care, everything is just a few taps away. By simply downloading the app to your smartphone, you can explore services, place a request, and have providers come straight to your home—anytime, anywhere. Last week at AXIS Residence, our Suntel Technology Cambodia team actively demonstrated the app to residents, allowing them to try out its features in real-time and experience firsthand how simple and convenient it is to book reliable services through bEasy.",
  // },
  // {
  //   id: 3,
  //   image: "/images/media/peng/p1.webp",
  //   images: [
  //         "/images/media/peng/p1.webp",
  //        "/images/media/peng/p2.webp",
  //        "/images/media/peng/p3.webp",
  //        "/images/media/peng/p4.webp",
  //        "/images/media/peng/p5.webp",
  //        "/images/media/peng/p6.webp",
  //        "/images/media/peng/p7.webp",
      
  //      ],
  //   date: "17 May 2025",
  //   title: "bEasy Event at Borey Peng Huoth Boeung Snor",
  //   description:
  //     "Here’s a look at memorable moments from Suntel Technology Cambodia’s booth during the International Mother’s Day event at Borey Peng Huoth Boeung Snor on May 10–11. The event provided a great opportunity for our team to introduce and demonstrate the bEasy App to the community. Attendees explored the app’s features, enjoyed complimentary ice cream, and experienced free nail art—making the celebration both engaging and informative.",
  // },
  // {
  //   id: 4,
  //   image: "/images/services/pest.webp",
  //   date: "24-25 / May / 2025",
  //   title: "Security Update & Patches",
  //   description:
  //     "Details on the latest security enhancements implemented to protect user data and ensure system integrity.",
  // },
  // {
  //   id: 5,
  //   image: "/images/services/uphol.webp",
  //   date: "24-25 / May / 2025",
  //   title: "Community Feedback Highlights",
  //   description:
  //     "A summary of the most impactful suggestions and feedback received from our active user community.",
  // },
  // {
  //   id: 6,
  //   image: "/images/services/washing.webp",
  //   date: "24-25 / May / 2025",
  //   title: "Summer Development Sprint",
  //   description:
  //     "Highlights from our recent two-week sprint, including new API endpoints and faster page load times.",
  // },
  // {
  //   id: 7,
  //   image: "/images/services/office.webp",
  //   date: "24-25 / May / 2025",
  //   title: "Celebrating Five Years in Business",
  //   description:
  //     "A look back at our journey, key accomplishments, and a special thank-you to all our supporters.",
  // },
  // {
  //   id: 8,
  //   image: "/images/services/office.webp",
  //   date: "24-25 / May / 2025",
  //   title: "Data Science Breakthrough",
  //   description:
  //     "Introducing our new machine learning model that improves recommendation accuracy by 25%.",
  // },
];